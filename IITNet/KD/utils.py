...
def load_checkpoint(ckpt, model, optimizer=None):
    if not os.path.exists(ckpt):
        raise(f"File doesn't exist {ckpt}")
...
def args_to_list(arg):
    args_list = arg.split(',')
    arg = list(map(int, args_list))
    
    return arg

def metric_result_fold(seq_len, y_trues, y_preds, task, model_type, fold, target):
    ...
    print(f'\n\n[INFO] task: {task}, model_type: {model_type}')
    print(f'Train target: {target}')
    if target == 'kd':
        print(f'\nsequence_length : teacher {seq_len[0]},  student {seq_len[0]}, Evaluation result from fold {fold}' )
    else: print(f'\nsequence_length : {target} {seq_len}, Evaluation result from fold 1 to 20' )
    print('\n' + overall_dt.table)
    print('\n' + perclass_dt.table)
    print(colored(' A', 'cyan') + ': Actual Class, ' + colored('P', 'green') + ': Predicted Class' + '\n\n')
    
def metric_result(seq_len, y_trues, y_preds, task, model_type, target):
    y_pred_argmax = np.argmax(y_preds, 1)
    result_dict = skmet.classification_report(y_trues, y_pred_argmax, digits=3, output_dict=True)
    cm = skmet.confusion_matrix(y_trues, y_pred_argmax)
    ...
    print(f'Train target: {target}')
    if target == 'kd':
        print(f'\nsequence_length : teacher {seq_len[0]},  student {seq_len[1]}, Evaluation result from fold 1 to 20' )
    else: print(f'\nsequence_length : {target} {seq_len}, Evaluation result from fold 1 to 20' )
    print('\n' + overall_dt.table)
    print('\n' + perclass_dt.table)
    print(colored(' A', 'cyan') + ': Actual Class, ' + colored('P', 'green') + ': Predicted Class' + '\n\n')
...
class ES():
    def __init__(self, ckpt_path, ckpt_file, patience = 10, verbose = True, delta = 0):
    ...
    def __call__(self, val_loss, model): 
        ...
        if self.best_score is None:
            ...
            self.save_checkpoint(self.ckpt_path, val_loss, model)
    ...
    def save_checkpoint(self, ckpt_path, val_loss, model):
