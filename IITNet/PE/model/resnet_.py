...
def conv3x1(in_planes, out_planes, padding = 1, stride = 1):
    return nn.Conv1d(in_planes, out_planes, kernel_size = 3, stride = stride, padding = padding, bias = False)
...
class ResNetFeature(nn.Module):
    ...
    def __init__(self, config, seq_len):
        ...
        self.layers = self.layer_dict[self.num_layers]
        ...
        if self.num_layers == 18 or self.num_layers == 34:
            ...
            raise NotImplementedError(f"num layers {config['NUM_LAYERS']} is not in layer config")
